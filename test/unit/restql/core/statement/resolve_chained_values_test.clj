...
(deftest resolve-chained-values-test
  ...
  (testing "Returns a statement with :empty as value if done-resource status code is not in 399 >= status >= 200"
    (is (= {:from :resource-name, :with {:id :empty}}
           (resolve-chained-values {:from :resource-name :with {:id [:done-resource :id]}}
                                   [[:done-resource {:status 400 :body "ERROR"}]])))

    (is (= {:from :resource-name :with {:id [:empty 2]}}
           (resolve-chained-values {:from :resource-name :with {:id [:done-resource :id]}}
